...
const { isObj, isArrayOf } = core.types;
...
function empty(container) {
  ...
  else if (isObj(container) && typeof container.clear === 'function') {
    container.clear();
  }
  else {
...
/**
 * Get the index (offset) of an Element inside its parent,
 * in relation to its sibling Elements.
 * @param {Element} el - Element we want the index of.
 * @returns {number}
 * @alias module:@lumjs/web-core/utils.indexOf
 */
function indexOf(el) {
  let i = 0;
...
/**
 * Get the index (offset) of a Node inside its parent,
 * in relation to its sibling Nodes.
 * @param {Node} node - Node we want the index of.
 * @returns {number}
 * @alias module:@lumjs/web-core/utils.indexOfNode
 */
...
// All main exports are defined here.
exports = module.exports = {
  empty, getNested, guessHTML, indexOf, indexOfNode,
  isCollection, isContainer, isQueryable,
  VALID_TAG, whenDOMReady, whenWindowReady,
}
